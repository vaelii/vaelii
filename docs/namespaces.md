...
src/vaelii/impl/
  ...
  wiring.clj        the other leaf extension point, and the whole inventory of it: the two calls that run *up* the layering — the assert path (for `nat` and `skolem`) and the prover registry (for `resolution`) — plus `import-dump`, the `predAllSpecified` audit, and the bounded integrity aggregate, layering inversions rather than recursions, and the `*defer-settle?*` flag both sides read.  Each entry, and why the set is collected here instead of left at the call sites, is "The layering" at the foot of this file
