...
;; KB (`vaelii.host.subscribe`, docs/feed.md).
;;
...
(defn- wrap-bearer-auth
  ...
  live in `vaelii.host.guard` for that reason."
  [handler token]
  (guard/wrap-bearer
...
(def ^:private loopback
  ...
  that machine is a local client too, which is what `vaelii.host.guard` is for, and
