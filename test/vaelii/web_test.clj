...
(deftest a-reader-can-read-only-what-the-kb-was-told
  ...
  (let [shown  (:body (GET "/term" "q=dog"))
    ...
    (is (< (count (re-seq #"data-h=" (re-find #"Argument position 1 .*?</ul>" (:body hid))))
           (stored (:body hid)))
        "which is more than the hidden group lists")
...
(deftest a-query-context-on-the-levels-page-is-a-400-and-not-a-500
  ...
  (testing "and a level the stack does not have is an empty page of rows, as an unreadable
            one is — the engine's `:bad-level` reached Jetty as a 500"
    (doseq [lvl ["8" "99" "-1"]]
      (let [r (GET "/levels/rows" (str "q=(animal%20%3Fx)&ctx=CxNaturalWorld&level=" lvl))]
        (is (= 200 (:status r)) lvl)
        (is (str/blank? (:body r)) lvl))))
    ...
    (try
      ...
      (testing "so does a number no port has, rather than overflowing an int or reaching Jetty"
        (doseq [v ["99999999999" "70000" "-5"]]
          ...
          (is (= (or env-port 3000) (#'web/default-port)) v)))
      (finally
