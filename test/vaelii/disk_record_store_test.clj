...
(defn- strip-premise-flags!
  "Rewrite `ids`' slots (every live slot when nil) with flags 0 — the form a store
...
(defn- strip-strength-flags!
  ...
  the form a store written after the premise bit but before the strength bits has.  That
...
(defn- corrupt-slot-flags!
  "Overwrite `id`'s sentex slot with `flags`, keeping its offset/length/gen — the form a
    ...
    (fn [dir]
      (let [s (drs/open-record-store dir)]
        (try
          ...
          (testing "and the justification half opens through the same entry point"
