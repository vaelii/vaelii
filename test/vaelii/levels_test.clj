...
(tu/deftest-kb lookup-rejects-a-level-off-the-ladder
  (is (thrown? clojure.lang.ExceptionInfo (v/lookup kb 8 '(dog Muffet) 'CxUniverse)))
  (is (thrown? clojure.lang.ExceptionInfo (v/lookup kb -1 '(dog Muffet) 'CxUniverse))))
  ...
  (tu/with-terms [dog Muffet CxStory]
    (let [h (v/assert kb (list dog Muffet) CxStory)]
      (testing "a sentence goal is turned into an index path"
        (is (= #{h} (set (map :handle (v/lookup kb 0 (list dog Muffet) CxStory))))))
      (testing "a vector goal *is* the path, addressing the index directly"
        (is (= #{h} (set (map :handle (v/lookup kb 0 [dog Muffet CxStory] nil))))))
      (testing "nothing is interpreted — no sentence, context or bindings"
        (let [r (first (v/lookup kb 0 (list dog Muffet) CxStory))]
          (is (= 0 (:level r)))
...
(tu/deftest-kb level-1-is-context-plus-functor-retrieval
  (tu/with-terms [parentOf Tom Bob Ann Rex CxStory CxOther]
    (v/assert kb (list parentOf Tom Bob) CxStory)
    (v/assert kb (list parentOf Ann Rex) CxStory)
    (v/assert kb (list parentOf Tom Bob) CxOther)
    (testing "arguments are not matched — this is candidate retrieval, not a match"
      (let [rs (v/lookup kb 1 (list parentOf Tom Bob) CxStory)]
        (is (= 2 (count rs)))                        ; (Ann Rex) comes back too
        (is (= #{CxStory} (set (map :context rs))))))
    (testing "the other context's copy is excluded — no inheritance at level 1"
      (is (= 1 (count (v/lookup kb 1 (list parentOf Tom Bob) CxOther)))))
    (testing "results carry a handle and the stored sentence, but no bindings"
      (let [r (first (v/lookup kb 1 (list parentOf Tom Bob) CxStory))]
        (is (integer? (:handle r)))
        (is (seq (:sentence r)))
        (is (nil? (:bindings r)))))))
...
(tu/deftest-kb level-2-unifies-within-one-context
  (tu/with-terms [parentOf Tom Bob Ann Rex CxStory]
    (v/assert kb (list parentOf Tom Bob) CxStory)
    (v/assert kb (list parentOf Ann Rex) CxStory)
    (testing "unification narrows what level 1 merely retrieved"
      (let [rs (v/lookup kb 2 (list parentOf Tom '?who) CxStory)]
        (is (= 1 (count rs)))
        (is (= Bob (get (:bindings (first rs)) '?who)))))
    (testing "a non-matching goal in the right context yields nothing"
      (is (empty? (v/lookup kb 2 (list parentOf Tom Rex) CxStory))))))

(tu/deftest-kb level-2-probes-the-symmetric-mirror
  (tu/with-terms [siblingOf Ann Carol CxStory]
    (v/assert kb (list 'symmetric siblingOf) CxStory)
    (v/assert kb (list siblingOf Ann Carol) CxStory)
    (testing "either argument order finds the one stored sentex"
      (is (= 1 (count (v/lookup kb 2 (list siblingOf '?x Carol) CxStory))))
      (is (= 1 (count (v/lookup kb 2 (list siblingOf Carol '?x) CxStory)))))))
...
(tu/deftest-kb query-core-is-exactly-level-2
  (tu/with-terms [parentOf siblingOf flies Tom Bob Ann Carol Opus CxStory]
    (v/assert kb (list 'symmetric siblingOf) CxStory)
    (v/assert kb (list parentOf Tom Bob)   CxStory {:strength :monotonic})
    (v/assert kb (list parentOf Tom Ann)   CxStory {:strength :monotonic})
    (v/assert kb (list parentOf Carol Ann) CxStory {:strength :monotonic})
    (v/assert kb (list siblingOf Ann Carol) CxStory {:strength :monotonic})
    (v/assert kb (list 'not (list flies Opus)) CxStory {:strength :monotonic})
    (letfn [(l2 [goal] (set (map :sentence (v/lookup kb 2 goal CxStory))))
            (qq [goal] (set (map :sentence (v/sentexes-matching kb goal CxStory))))]
...
(tu/deftest-kb query-adds-the-except-visibility-filter
  ...
  (tu/with-terms [likes Bob Ann CxStory]
    (let [h (v/assert kb (list likes Bob Ann) CxStory {:strength :monotonic})]
      (testing "with no except, query and level 2 both see the fact"
        (is (= 1 (count (v/sentexes-matching kb (list likes Bob Ann) CxStory))))
        (is (= 1 (count (v/lookup kb 2 (list likes Bob Ann) CxStory)))))
      (let [eh (v/assert kb (list 'except (sx/sentex-handle h)) CxStory {:strength :monotonic})]
        (testing "query drops the excepted fact; level 2 (no except filter) keeps it"
          (is (empty? (v/sentexes-matching kb (list likes Bob Ann) CxStory)))
          (is (= 1 (count (v/lookup kb 2 (list likes Bob Ann) CxStory)))))
...
(tu/deftest-kb query-rewrites-a-retired-spelling-that-level-2-does-not
  ...
  (tu/with-terms [likes Bob Ann Annie CxStory]
    (v/assert kb (list likes Bob Annie) CxStory {:strength :monotonic})
    (v/assert kb (list 'rewriteOf Ann Annie) CxStory {:strength :monotonic}) ; Annie deprecated -> Ann
    ...
    (testing "query rewrites the retired spelling in the goal and still finds it"
      (is (= 1 (count (v/sentexes-matching kb (list likes Bob Annie) CxStory)))))
    (testing "level 2 does not rewrite — the retired spelling is superseded, so it finds nothing"
      (is (empty? (v/lookup kb 2 (list likes Bob Annie) CxStory))))))

;; ---- level 3: context inheritance via genlCx -----------------------

(tu/deftest-kb level-3-adds-genlCx-inheritance
  (tu/with-terms [parentOf Tom Bob CxSub CxSuper]
    ...
    (testing "level 2 sees only the literal context and misses the inherited fact"
      (is (empty? (v/lookup kb 2 (list parentOf Tom '?y) CxSub))))
    (testing "level 3 sees it through the up-closure, and reports where it is stored"
      (let [rs (v/lookup kb 3 (list parentOf Tom '?y) CxSub)]
        (is (= 1 (count rs)))
        (is (= Bob (get (:bindings (first rs)) '?y)))
        (is (= CxSuper (:context (first rs))))))))
...
(tu/deftest-kb level-4-adds-the-genl-spec-walk
  (tu/with-terms [dog animal Muffet CxStory]
    ...
    (testing "no supertype fact was materialized to make that work"
      (is (empty? (v/lookup kb 2 (list animal Muffet) CxStory))))))
...
(tu/deftest-kb level-7-adds-backchaining
  (tu/with-terms [parentOf grandparentOf Tom Bob Ann CxStory]
    ...
    (testing "no level below 7 derives through the rule"
      (doseq [n (range 0 7)]
        (is (empty? (v/lookup kb n (list grandparentOf Tom '?who) CxStory))
            (str "level " n " should not backchain"))))
    (testing "level 7 does"
      (let [rs (v/lookup kb 7 (list grandparentOf Tom '?who) CxStory)]
  ...
  (tu/with-terms [parentOf Tom Bob CxStory]
    ...
    (testing "the fact prover is in the level-6 stack"
      (is (= #{Bob} (set (map #(get (:bindings %) '?y)
                              (v/lookup kb 6 (list parentOf Tom '?y) CxStory))))))
    (testing "level 7 agrees — backchaining only ever adds"
      (is (= (map :bindings (v/lookup kb 6 (list parentOf Tom '?y) CxStory))
             (map :bindings (v/lookup kb 7 (list parentOf Tom '?y) CxStory)))))))
...
(tu/deftest-kb every-level-returns-the-same-map-shape
  (tu/with-terms [dog animal Muffet CxStory]
    ...
    (v/assert kb (list dog Muffet) CxStory)
    (doseq [n (range 8)]
      (doseq [r (v/lookup kb n (list dog Muffet) CxStory)]
...
(tu/deftest-kb escalate-stops-at-the-cheapest-level-that-answers
  (tu/with-terms [dog animal Muffet CxStory]
    ...
    (testing "a goal needing the spec walk escalates to level 4 and stops"
      (let [r (v/escalate kb (list animal '?x) CxStory)]
        (is (= 4 (:level r)))
        (is (= :typed (:name r)))
    ...
    (testing "a goal a plain match answers stops at level 2"
      (is (= 2 (:level (v/escalate kb (list dog Muffet) CxStory)))))
    (testing "an explicit floor of 0 admits the retrieval levels"
      (is (= 0 (:level (v/escalate kb (list dog Muffet) CxStory 0)))))
    (testing "an unanswerable goal reports no level, having tried them all"
      (tu/with-terms [cat]
        (let [r (v/escalate kb (list cat '?x) CxStory)]
          (is (nil? (:level r)))
...
(tu/deftest-kb escalate-skips-the-retrieval-levels-by-default
  ...
  (tu/with-terms [ancestorOf Ann Bob Carol CxStory]
    ...
    (testing "the transitive goal is genuinely answered by the closure, at level 5"
      (is (= 5 (:level (v/escalate kb (list ancestorOf Ann Carol) CxStory)))))
    (testing "from floor 0, level 1 claims it — on the strength of a different fact"
      (is (= 1 (:level (v/escalate kb (list ancestorOf Ann Carol) CxStory 0)))))))
...
(tu/deftest-kb explain-levels-shows-where-the-answer-appears
  (tu/with-terms [dog animal Muffet CxStory]
    (v/assert kb (list 'genl dog animal) CxStory)
    (v/assert kb (list dog Muffet) CxStory)
    (let [rows (v/explain-levels kb (list animal '?x) CxStory)
...
(tu/deftest-kb the-answer-count-never-falls-as-the-stack-is-climbed
  ...
  (tu/with-terms [dog animal siblingOf ancestorOf barks Muffet Ann Bob Carol
    ...
    (v/assert kb (list dog Muffet) CxStory)
    (v/assert kb (list 'symmetric siblingOf) CxStory)
    (v/assert kb (list siblingOf Ann Bob) CxStory)
    (v/assert kb (list 'transitive ancestorOf) CxStory)
    (v/assert kb (list ancestorOf Ann Bob) CxStory)
    (v/assert kb (list ancestorOf Bob Carol) CxStory)
    (v/assert-rule kb [(list dog '?x)] (list barks '?x) CxStory)
    (doseq [ctx  [CxStory CxSub]
...
(tu/deftest-kb an-excepted-fact-is-the-one-answer-that-falls-out-of-the-stack
  ...
  (tu/with-terms [likes Bob Ann CxStory]
    (let [h (v/assert kb (list likes Bob Ann) CxStory {:strength :monotonic})]
      (v/assert kb (list 'except (sx/sentex-handle h)) CxStory {:strength :monotonic})
      (let [goal (list likes Bob Ann)]
        (testing "levels 2 and 3 match it; level 4 and everything above drop it"
          (is (= 1 (count (v/lookup kb 2 goal CxStory))))
          (is (= 1 (count (v/lookup kb 3 goal CxStory))))
          (is (every? #(empty? (v/lookup kb % goal CxStory)) (range 4 8))))
        (testing "the engine's own answer is the one level 4 gives"
          (is (empty? (v/ask kb goal CxStory))))
...
(tu/deftest-kb level-3-reports-one-fact-once-however-many-names-it-has
  ...
  (tu/with-terms [dog Muffet Rex CxStory CxSub]
    (v/assert kb (list 'genlCx CxSub CxStory) CxStory)
    (v/assert kb (list dog Rex) CxStory {:strength :monotonic})
    (v/assert kb (list 'sameAs Muffet Rex) CxSub {:strength :monotonic})
    (let [goal (list dog '?x)
          rep  (v/representative kb Rex CxSub)]
...
(tu/deftest-kb level-5-folds-a-derived-answer-into-the-stored-one-it-repeats
  ...
  (tu/with-terms [ancestorOf Ann Bob Carol CxStory CxSub]
    (v/assert kb (list 'genlCx CxSub CxStory) CxStory)
    (v/assert kb (list 'transitive ancestorOf) CxStory)
    (v/assert kb (list ancestorOf Ann Bob) CxStory)
    (v/assert kb (list ancestorOf Bob Carol) CxStory)
    (testing "the ground goal the store already answers is one result, not two"
      (let [rs (v/lookup kb 5 (list ancestorOf Ann Bob) CxSub)]
        (is (= 1 (count rs)))
    ...
    (testing "the open goal answers once per answer, the closure link included"
      (let [rs (v/lookup kb 5 (list ancestorOf Ann '?x) CxSub)]
        (is (= #{Bob Carol} (set (map #(get (:bindings %) '?x) rs))))
        (is (= 2 (count rs)))
        (testing "which is what level 6 counts too"
          (is (= (count (v/lookup kb 6 (list ancestorOf Ann '?x) CxSub))
                 (count rs))))))))
...
(tu/deftest-kb level-5-never-drops-a-stored-match-to-a-derived-one
  ...
  (tu/with-terms [ancestorOf Ann Bob CxStory CxOther]
    (v/assert kb (list 'transitive ancestorOf) CxStory)
    (v/assert kb (list ancestorOf Ann Bob) CxStory)
    (v/assert kb (list ancestorOf Ann Bob) CxOther)
...
(tu/deftest-kb taking-one-result-costs-fewer-record-fetches-than-taking-all
  (tu/with-terms [parentOf Tom CxStory]
    ...
    (let [kids (vec (repeatedly 80 tu/tmp-ind))]
      (doseq [k kids] (v/assert kb (list parentOf Tom k) CxStory {:chain? false}))
      ...
      (doseq [level [1 2 3 4]]
        (let [n     (atom 0)
              ...
              _     (first (v/lookup probe level goal CxStory))
              one   @n
              _     (reset! n 0)
              _     (dorun (v/lookup probe level goal CxStory))
              all   @n]
...
(tu/deftest-kb level-5-does-not-run-the-closure-for-a-result-the-store-answers
  ...
  (tu/with-terms [ancestorOf CxStory CxSub]
    ...
    (let [n (fn [i] (symbol (str "TmpAnc" i)))]
      (doseq [i (range 20)]
        (v/assert kb (list ancestorOf (n i) (n (inc i))) CxStory {:chain? false}))
      (let [calls (fn [f] (let [c (atom 0), orig provers/solve-goal-with]
        ...
        (testing "one result comes from the store, and never reaches a prover"
          (is (some? (first (v/lookup kb 5 goal CxSub))))
          (is (zero? (calls #(first (v/lookup kb 5 goal CxSub))))))
        (testing "the whole answer set does run the closure"
          (is (pos? (calls #(dorun (v/lookup kb 5 goal CxSub))))))))))
...
(tu/deftest-kb level-7-streams-its-search-rather-than-running-it-to-completion
  ...
  ;; to the registry.
  (tu/with-terms [parentOf anc CxStory]
    (let [n (fn [i] (symbol (str "TmpLzChain" i)))]
      (doseq [i (range 30)]
        (v/assert kb (list parentOf (n i) (n (inc i))) CxStory {:chain? false}))
      (v/assert-rule kb [(list parentOf '?x '?z)] (list anc '?x '?z) CxStory
                     {:direction :backward})
      (v/assert-rule kb [(list parentOf '?x '?y) (list anc '?y '?z)] (list anc '?x '?z)
                     CxStory {:direction :backward})
      (let [goal  (list anc (n 0) '?z)
            calls (fn [f] (let [c (atom 0), orig provers/solve-goal]
                            (with-redefs [provers/solve-goal
                              ...
                              (f))
                            @c))
            cost  (fn [k] (calls #(dorun (take k (v/lookup kb 7 goal CxStory)))))
            all   (calls #(dorun (v/lookup kb 7 goal CxStory)))]
        (testing "the search really does have a tail to avoid paying for"
          (is (= 30 (count (v/lookup kb 7 goal CxStory)))
  ...
  (tu/with-terms [parentOf Tom Bob CxStory]
    ...
    (let [invoked (atom false)
      ...
      (testing "taking one answer never reaches the expensive prover's solve"
        (is (some? (first (v/lookup kb 6 (list parentOf Tom '?y) CxStory))))
        (is (false? @invoked)))
      (testing "taking every answer does"
        (dorun (v/lookup kb 6 (list parentOf Tom '?y) CxStory))
        (is (true? @invoked))))))
...
(tu/deftest-kb an-out-of-range-floor-is-refused-not-answered-empty
  ...
  (tu/with-terms [pp Aa CxLv]
    (v/assert kb (list pp Aa) CxLv)
    (doseq [floor [8 99 -1 :two]]
      (let [e (try (v/escalate kb (list pp Aa) CxLv floor) nil
